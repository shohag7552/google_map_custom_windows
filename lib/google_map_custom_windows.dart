...
class _CustomMapInfoWindowState extends State<CustomMapInfoWindow> {
  ...
  void _updateInfoWindow() async {
    ...
    for(LatLng? latLng in _latLng!) {
      ScreenCoordinate screenCoordinate = await widget.controller.googleMapController!.getScreenCoordinate(latLng!);
      left.add((screenCoordinate.x.toDouble() / devicePixelRatio) - ((widget.width/2) + widget.offset.dx));
      top.add((screenCoordinate.y.toDouble() / devicePixelRatio) - (widget.offset.dy + widget.height));
    }
    setState(() {
      _showNow = true;
      _leftMargin = left;
  ...
  Widget build(BuildContext context) {
    if(_latLng == null || _latLng!.isEmpty || _childs == null || _childs!.isEmpty) {
      return const SizedBox.shrink();
    }
    return Stack(children: _latLng!.map((e) {
      return Positioned(
        left: _leftMargin[_latLng!.indexOf(e)],
        top: _topMargin[_latLng!.indexOf(e)],
        child: Visibility(
          visible: (_showNow == false || (_leftMargin[_latLng!.indexOf(e)] == 0 && _topMargin[_latLng!.indexOf(e)] == 0) || _childs == null || e == null)
              ? false : true,
          child: SizedBox(
